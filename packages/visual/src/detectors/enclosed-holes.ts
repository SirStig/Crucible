 ...
 * documented limitation, not proven computer vision (see
 * packages/visual/RESEARCH.md).
 */
export function detectEnclosedHoles(grid: PixelGrid, options: VisualGradeOptions = {}): Finding[] {
  ...
  for (const hole of holes) {
    ...
    findings.push({
      ...
      severity: "warn",
      message: `A ${hole.pixels.length}px transparent gap is fully sealed inside the silhouette — likely an accidental hole in the fill rather than a deliberate opening.`,
