...
import type { Finding, FindingLocation } from "../../base/index.js";
import { loadVisualCraftRubricData } from "../../visual/index.js";
...
export function gradeSpriteCraftHandler(input: GradeSpriteCraftInput) {
  ...
  if (!entry) {
    throw new Error(
      `Crucible: no Tier 2 rubric item named "${input.ruleId}". Call get_sprite_craft_rubric first to see available items.`,
    );
  }
